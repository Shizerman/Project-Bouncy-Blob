...
const NeonSlime = () => {
  ...
  const initializeGame = () => {
    ...
    for (let i = 0; i < 30; i++) {
      const y = 600 - i * 150 - Math.random() * 50;
  ...
  useEffect(() => {
    ...
    const handleKeyUp = (e) => {
      ...
      // Jump release
      if (e.key === ' ' && state.player.state === 'grounded') {
    ...
    const update = (deltaTime) => {
      ...
      state.camera.y += (targetCameraY - state.camera.y) * 0.05;
      ...
      if (highestPlatform > state.camera.y - 800) {
        const spacing = 150 / state.difficulty;
        const newY = highestPlatform - spacing - Math.random() * 50;
      ...
      state.platforms = state.platforms.filter(platform => platform.y < state.camera.y + 800);
      state.trampolines = state.trampolines.filter(t => t.y < state.camera.y + 800);
      state.sparks = state.sparks.filter(s => s.y < state.camera.y + 800);
